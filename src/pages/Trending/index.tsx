...
export default function Trending() {
  ...
  useEffect(() => {
    setLoading(true)
    const fetch = mediaType === 'tv'
      ? tmdb.tvTrending(timeWindow).then(d => (d.results as TVShow[]).map(normalizeTVShow))
      : tmdb.trending(timeWindow).then(d => d.results as Movie[])
    fetch
      .then(setMovies)
      .catch(console.error)
...
function TrendingHero({ movie, timeWindow, onToggle, mediaType, onMediaTypeToggle }: { movie: Movie; timeWindow: 'day' | 'week'; onToggle: (v: 'day' | 'week') => void; mediaType: 'movie' | 'tv'; onMediaTypeToggle: (v: 'movie' | 'tv') => void }) {
  ...
  return (
    <div style={{ position: 'relative', height: 620, overflow: 'hidden' }}>
      {backdrop && (
        <img src={backdrop} alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', filter: 'brightness(0.38)' }} />
      )}
      ...
      <div style={{ position: 'relative', zIndex: 10, maxWidth: 1280, margin: '0 auto', padding: '0 20px', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', paddingBottom: 60 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 20 }}>
          <div style={{ display: 'flex', border: '1px solid rgba(255,255,255,0.15)', overflow: 'hidden' }}>
            {(['movie', 'tv'] as const).map(mt => (
              <button
                key={mt}
                onClick={() => onMediaTypeToggle(mt)}
                style={{ padding: '7px 18px', backgroundColor: mediaType === mt ? 'var(--accent)' : 'transparent', color: mediaType === mt ? 'var(--accent-on)' : 'rgba(255,255,255,0.5)', border: 'none', fontSize: 11, letterSpacing: '0.15em', fontWeight: mediaType === mt ? 600 : 400, cursor: 'pointer', transition: 'all 0.2s' }}
              >
                {mt === 'movie' ? '영화' : 'TV'}
              </button>
            ))}
          ...
          <div style={{ display: 'flex', border: '1px solid rgba(255,255,255,0.15)', overflow: 'hidden' }}>
            {(['week', 'day'] as const).map(tw => (
              <button
                key={tw}
                onClick={() => onToggle(tw)}
                style={{ padding: '7px 18px', backgroundColor: timeWindow === tw ? 'rgba(255,255,255,0.15)' : 'transparent', color: timeWindow === tw ? '#fff' : 'rgba(255,255,255,0.5)', border: 'none', fontSize: 11, letterSpacing: '0.15em', fontWeight: timeWindow === tw ? 600 : 400, cursor: 'pointer', transition: 'all 0.2s' }}
              >
                {tw === 'week' ? '이번 주' : '오늘'}
              </button>
            ))}
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          <button
            ...
            style={{ backgroundColor: A, color: 'var(--accent-on)', border: 'none', padding: '12px 28px', fontSize: 12, letterSpacing: '0.15em', fontWeight: 600, cursor: 'pointer', transition: 'background-color 0.2s' }}
